...
class Pathfinding:
    ...
    def __init__(self, Start, Goal, Costs) -> None:
        self.costs = Costs
    ...
    def lowest_node(self, list, goal): # get the node with the lowest cost
    ...
    def final_path(self, node):  # returns the final path by going throuhg all of the parent nodes and creating a new list which will be sent
        path = []
    ...
    def get_neighbours(self, current_node):
        ...
        neighbours.append(Node((current_node.tile[0], current_node.tile[1] - 1)))
        ...
        while len(open_List) is not 0:
            ...
            for i in range(len(neighbour)):
                ...
                for j in range(len(closed_List)):
                    ...
                    if neighbour[i].tile == closed_List[j].tile: # check if the tile value from one of the neighbours is present in the close list

                        present = True


                if present:     # if it is present pass onto the next one
                    pass
                else:
